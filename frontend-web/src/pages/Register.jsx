...
const Register = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    role: '' // No default - user must explicitly select
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.fullName) {
        sessionStorage.setItem('pendingName', formData.fullName);
      }
  ...
  const handleGoogleSignUp = async () => {
    ...
    if (!formData.role) {
      setError('Please select your role (Student or Teacher) before signing in with Google');
      return;
    }
    ...
    try {
      ...
      // Register/login with selected role
      ...
      <main className="flex-grow flex items-center justify-center py-12 px-6 bg-pattern">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <h1 className="text-[#150d1c] dark:text-white tracking-tight text-[32px] font-bold leading-tight mb-2">Create your account</h1>
            ...
            <div className="p-8">
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      disabled={isLoading}
                    />
                    <span className="material-symbols-outlined absolute right-4 top-3 text-[#79479e]/60 text-[20px]">person</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      disabled={isLoading}
                    />
                    <span className="material-symbols-outlined absolute right-4 top-3 text-[#79479e]/60 text-[20px]">mail</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[#150d1c] dark:text-gray-200 text-sm font-medium mb-1.5 ml-1">Password</label>
                    <input
                      className="w-full rounded-lg text-[#150d1c] dark:text-white border border-[#ddcee9] dark:border-[#3a264a] bg-white dark:bg-[#1a0f23] focus:ring-2 focus:ring-primary/20 focus:border-primary h-12 px-4 text-base font-normal placeholder:text-[#79479e]/50 outline-none transition-all"
                      placeholder="••••••••"
                      type="password"
                  ...
                  <div>
                    <label className="block text-[#150d1c] dark:text-gray-200 text-sm font-medium mb-1.5 ml-1">Confirm Password</label>
                    <input
                      className="w-full rounded-lg text-[#150d1c] dark:text-white border border-[#ddcee9] dark:border-[#3a264a] bg-white dark:bg-[#1a0f23] focus:ring-2 focus:ring-primary/20 focus:border-primary h-12 px-4 text-base font-normal placeholder:text-[#79479e]/50 outline-none transition-all"
                      placeholder="••••••••"
                      type="password"
              ...
              {!formData.role && (
                <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg flex items-start gap-2">
                  ...
                  <p className="text-sm text-blue-700 dark:text-blue-300">
                    Please select your role (Student or Teacher) above before signing in with Google.
                  </p>
                </div>
              )}
